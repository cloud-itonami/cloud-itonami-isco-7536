(ns shoecoord.actor
  "ShoeCoordActor — the ISCO-08 7536 shoemakers workshop scheduling/
  logistics coordination actor as a `langgraph.graph/state-graph`
  (ADR-2607121000 / CLAUDE.md Actors section). One graph run = one
  workshop-coordination operation request (intake -> advise -> govern
  ...
  Modeled closely on cloud-itonami-isco-7522's cabinetcoord.actor.
  ...
  The unconditional invariant: the Shoemaking Workshop Coordination
  Advisor can never directly commit an operating record the
  ShoeCoordGovernor refuses, and can never make this actor finalize a
  ...
  (e.g. declaring the workshop safety cleared for operation), and can
  never override a shop safety officer's judgment — every
  ...
  This actor coordinates WORKSHOP SCHEDULING/LOGISTICS ONLY — it never
  performs shoemaking work itself, and it never makes a
...
(defn build-graph
  "Build a compiled ShoeCoordActor graph. `store` implements
  `shoecoord.store/Store`. `advisor` implements
  `shoecoord.advisor/Advisor` (defaults to `mock-advisor`).
